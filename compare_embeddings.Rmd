...
# all_embeddings <- read.csv('MPNET Output.csv')
...
```{r separate conditions - START HERE}
# baseline subjects 
baseline <- all_data %>%
...
byWords %>% ggplot() +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("darkorange3","deepskyblue3")) +
...
# t test for median
byWordsMedian <- all_data %>%
  filter(noMemory == 'false') %>%
  mutate(numWords = str_count(responses, "\\w+"),
         Condition = recode(Condition, "Baseline" = "Non-Competitive")) %>%
  group_by(Condition, subjNum) %>%
  summarize(medianWords = median(numWords))

tTestMedian = t.test(byWordsMedian$medianWords[byWordsMedian$Condition == "Competitive"],
       byWordsMedian$medianWords[byWordsMedian$Condition == "Non-Competitive"])
tTestMedian
# cohen's d for bias
cohensDMedian <- cohen.d(byWordsMedian$medianWords[byWordsMedian$Condition == "Competitive"],
       byWordsMedian$medianWords[byWordsMedian$Condition == "Non-Competitive"])
cohensDMedian
...
resultsPlot <- allSubjDF %>% ggplot() +
  ...
  theme_minimal() +
  scale_color_manual(values = c("darkorange4","deepskyblue4")) +
  scale_fill_manual(values = c("darkorange3","deepskyblue3")) +
  theme(axis.text.x = element_text(size=20),
        axis.text.y = element_text(size=20),
        axis.title.y = element_blank(),
